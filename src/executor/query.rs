...
mod postgres_array {
    ...
    impl TryGetable for pgvector::Vector {
        ...
        fn try_get_by<I: ColIdx>(res: &QueryResult, idx: I) -> Result<Self, TryGetError> {
            match &res.row {
                #[cfg(feature = "sqlx-mysql")]
                QueryResultRow::SqlxMySql(_) => Err(type_err("Vector unsupported by sqlx-mysql").into()),
                #[cfg(feature = "sqlx-postgres")]
                QueryResultRow::SqlxPostgres(row) => row
                    ...
                    .and_then(|opt| opt.ok_or_else(|| err_null_idx_col(idx))),
                #[cfg(feature = "sqlx-sqlite")]
                QueryResultRow::SqlxSqlite(_) => Err(type_err("Vector unsupported by sqlx-sqlite").into()),
                #[cfg(feature = "mock")]
                QueryResultRow::Mock(row) => {
